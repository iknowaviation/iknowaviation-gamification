...
// - single  => Watu answer_type=radio
// - multi   => Watu answer_type=checkbox (strict: requires 2+ correct)
// - boolean => Watu answer_type=radio + truefalse=1 (expects True/False choices)
$qtype = $qtype_raw;
if ( in_array( $qtype_raw, ['truefalse','true_false','tf'], true ) ) $qtype = 'boolean';
$answer_type = ( $qtype === 'multi' ) ? 'checkbox' : 'radio';
...
// Validate choice correctness counts based on question type.
$correct_count = 0;
foreach ( $choices as $cc0 ) {
	if ( is_array($cc0) && ! empty($cc0['correct']) ) $correct_count++;
}
if ( $qtype === 'single' || $qtype === 'boolean' ) {
	if ( $correct_count !== 1 ) {
		throw new Exception( 'Invalid correct choice count (expected exactly 1) at quizzes['.$qi.'].questions['.$i.'].' );
	}
}
if ( $qtype === 'multi' ) {
	if ( $correct_count < 2 ) {
		throw new Exception( 'Invalid multi question (requires 2+ correct choices) at quizzes['.$qi.'].questions['.$i.'].' );
	}
}

// Boolean questions: enforce True/False choices (case-insensitive).
$truefalse_flag = 0;
if ( $qtype === 'boolean' ) {
	$truefalse_flag = 1;
	$labels = array_map( function($c){ return is_array($c) && isset($c['a']) ? strtolower(trim((string)$c['a'])) : ''; }, $choices );
	$has_true  = in_array( 'true', $labels, true );
	$has_false = in_array( 'false', $labels, true );
	if ( ! $has_true || ! $has_false || count($choices) !== 2 ) {
		// Normalize to strict True/False set while preserving which option was marked correct.
		$w_correct_true = false;
		foreach ( $choices as $cc1 ) {
			$lab = is_array($cc1) && isset($cc1['a']) ? strtolower(trim((string)$cc1['a'])) : '';
			if ( $lab === 'true' && ! empty($cc1['correct']) ) $w_correct_true = true;
		}
		$choices = [
			[ 'a' => 'True',  'correct' => $w_correct_true ],
			[ 'a' => 'False', 'correct' => ! $w_correct_true ],
		];
		$log[] = 'WARNING: Boolean question choices were normalized to True/False (exactly 2 choices).';
	}
}
