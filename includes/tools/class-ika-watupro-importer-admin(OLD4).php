...
class IKA_WatuPRO_Importer_Admin {
	...
	public static function render_page() {
			...
			// Option fallback: if a transient is missed (object cache/transient issues),
			// use the saved option payload to show the last result message ONCE.
			$last_saved = isset($last_saved) ? $last_saved : get_option( 'ika_watupro_import_last_result_option' );
			$shown_ts   = (int) get_option( 'ika_watupro_import_last_result_shown_ts', 0 );

			if ( ! $last && is_array( $last_saved ) && ! empty( $last_saved['message'] ) ) {
				$payload_ts = (int) ( $last_saved['ts'] ?? 0 );
				// If we've already shown a notice for this payload (or newer), don't show again.
				if ( ! $payload_ts || $shown_ts < $payload_ts ) {
					$last = $last_saved;
					// Mark as shown so refreshes don't repeat the notice.
					update_option( 'ika_watupro_import_last_result_shown_ts', $payload_ts ? $payload_ts : time(), false );
				}
			}
...
<hr />
	...
	public static function handle_import() {
		...
		register_shutdown_function( function() use ( &$log ) {
			...
			// Reset 'shown' marker so the next page load shows the notice once.
			delete_option( 'ika_watupro_import_last_result_shown_ts' );
		} );
		// --- END FAIL-SAFE ---
